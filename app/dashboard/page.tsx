...
export default async function DashboardPage() {
    ...
    const activityData = last7Days.map(date => {
        const count = recentPatients?.filter((p: any) => p.created_at.startsWith(date)).length || 0
        return { date, count }
    })

    // Calculate Staff Counts
    const doctorCount = staff?.filter((m: any) => m.role === 'doctor').length || 0
    const nurseCount = staff?.filter((m: any) => m.role === 'nurse').length || 0
    const receptionistCount = staff?.filter((m: any) => m.role === 'receptionist').length || 0
